...
case ${TEST_SUITE} in
    ...
    rector)
        if [ "${CGLCHECK_DRY_RUN}" -eq 1 ]; then
            COMMAND=(php -dxdebug.mode=off .Build/bin/rector -n --config=Build/rector.php --clear-cache "$@")
        else
            COMMAND=(php -dxdebug.mode=off .Build/bin/rector --config=Build/rector.php --clear-cache "$@")
        fi
